...
dataBaseDir = 'G:\My Drive\Desktop\Image processing-code\msckf-swf-comparison\2011_09_26_drive_0005_sync\2011_09_26\2011_09_26_drive_0005_sync';
dataCalibDir = 'G:\My Drive\Desktop\vison tests\cell phone test 2\frames\2011_09_26_calib\2011_09_26';
...
T_cam2velo=T_camvelo_struct{2}; % For the second camera
T_veloimu = loadCalibrationRigid(fullfile(dataCalibDir,'calib_imu_to_velo.txt'));
T_camimu = T_camvelo*T_veloimu;
T_cam2imu= T_cam2velo*T_veloimu; % FOr the second camera
...
%     keyboard
   ...
   elseif detectNewPoints
        ...
        %RANSAC
        [~,inlierPtsPastLeft,inlierPtsCurrentLeft] = estimateGeometricTransform(lastLeftPoints,validLeftPoints,...
            'similarity', 'MaxDistance', ransacMaxDistance);
  ...
  t_tracking(image_i)=toc;   
%            showMatchedFeatures(viLeftImage, viRightImage, trackedLeftPoints, trackedRightPoints);
show(viLeftImage, viRightImage, trackedLeftPoints, trackedRightPoints);
%            figure(2)
% hold off;
%            showMatchedFeatures(viLeftImage, viRightImage, trackedLeftPoints, trackedRightPoints,'montage');
%            keyboard
   end
    %=====Temporal tracking======
end
...
R_c2i=T_cam2imu(1:3,1:3);
R_ci = T_camimu(1:3,1:3);
T_imucam = inv(T_camimu);
T_imucam2 = inv(T_cam2imu);
p_ci_i = T_imucam(1:3,4);
p_c2i_i = T_imucam2(1:3,4);
...
save([fileName], 'r_i_vk_i','theta_vk_i','w_vk_vk_i','v_vk_vk_i', 'cu','cv','fu','fv','b', 'y_k_j', 'C_c_v','C_c2_v', 'rho_v_c_v','rho_v_c2_v', 't');
